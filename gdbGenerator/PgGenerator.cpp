...
bool PgGenerator::generateGdbFile() {
    try {
		soci::session sql(_backEnd, m_connString);
		string fmtStrSql = "select {} as wkt from \"{}\" limit 1";
...
bool PgGenerator::createResultTable()
{
	try
	{
//		soci::session sql(*_poolList[0]);
		soci::session sql(_backEnd, m_connString);
		string strSql =
			fmt::format("create table {} ({} INTEGER not null, WKT TEXT not null, SCORE FLOAT not null)",m_resultTable, idField);
		sql << strSql;
...
bool PgGenerator::batchInsert(vector<std::string>& sqlList)
{
	initDB();
	soci::session sql(*_poolList[0]);
	try
